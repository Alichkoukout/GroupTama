...
export default function Hero() {
  ...
  function FloatingBadge({ icon: Icon, label, className }) {
    return (
      <div className={`glass-modern rounded-2xl px-4 py-3 flex items-center gap-3 ${className}`}>
        <div className="w-9 h-9 rounded-xl bg-primary-500/20 flex items-center justify-center flex-shrink-0">
          <Icon className="text-primary-300 text-lg" />
        </div>
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      ...
      {/* ── Accent left stripe ────────────────────────────────── */}
      <div className="absolute left-0 top-0 h-full w-[3px] bg-gradient-to-b from-transparent via-primary-500 to-transparent" />
      ...
      <div className="absolute -right-12 top-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full border border-primary-500/[.08] pointer-events-none" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 pt-28 pb-20 w-full">
        <div className="max-w-3xl">
          ...
          <div className="anim-badge inline-flex items-center gap-2.5 rounded-full px-5 py-2.5 mb-8
                          border border-primary-500/40 bg-primary-500/10 text-primary-300 text-xs font-black uppercase tracking-[.16em]">
            <HiOutlineLightningBolt className="text-primary-400 text-sm" />
            {t('hero.badge')}
          </div>
          ...
          <div className="anim-4 grid grid-cols-2 sm:grid-cols-4 gap-5">
            {stats.map(({ value, label }, i) => (
              <div key={label} className="relative pl-5 border-l-2 border-primary-500/40 hover:border-accent-400 transition-all group">
                <div className="text-3xl font-black text-white group-hover:text-accent-300 transition-colors">
                  {value}
                </div>
      ...
      <div className="absolute bottom-8 left-16 flex items-center gap-3 animate-bounce">
        <div className="flex flex-col gap-1">
          <div className="w-0.5 h-6 bg-gradient-to-b from-primary-400/80 to-transparent rounded mx-auto" />
        </div>
